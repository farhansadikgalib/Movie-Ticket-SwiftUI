            ...
            HStack(spacing: 0.0){
                ...
                .background(alignment: .leading){
                    Circle()
                        ...
                        .overlay(Circle().trim(from: 0, to: CGFloat(0.5))
                            .stroke(LinearGradient(colors: gradientCircle, startPoint: .top, endPoint: .bottom),style: StrokeStyle(lineWidth: 2)).rotationEffect(.degrees(135)).frame(width: 76, height: 76)
                            .offset(x: indicatorOffset(width: width), y: -17)
                        )
                }
