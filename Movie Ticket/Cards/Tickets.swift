...
struct InfiniteStackView: View{
    ...
    var body: some View {
        ...
        .offset(x: offset)
        .rotationEffect(getIndex() == 1 ? .degrees(-7) : .degrees(0) )
        .rotationEffect(getIndex() == 2 ? .degrees(7) : .degrees(0) )
        ...
        .gesture(DragGesture().updating($isDraging, body: {_, out ,_ in
            out = true
        })
            .onChanged({
    ...
    func getIndex() -> CGFloat {
        let index = tickets.firstIndex{
            ticket in return self.ticket.id == ticket.id
        } ?? 0
